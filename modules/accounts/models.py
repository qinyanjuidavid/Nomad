...
class User(AbstractBaseUser, TrackingModel):
    ...
    phone = PhoneNumberField(
        _("phone number"), unique=True, blank=True, null=True, max_length=27
    )
...
class Profile(models.Model):
    ...
    county = models.ForeignKey(
        Counties, on_delete=models.DO_NOTHING, blank=True, null=True
    )
    bio = models.TextField(_("bio"), blank=True, null=True)
    gender = models.CharField(
        _("gender"), choices=gender_choices, max_length=20, default="Male"
    )
    date_of_birth = models.DateField(_("date of birth"), blank=True, null=True)
    timestamp = models.DateTimeField(_("timestamp"), auto_now_add=True)
    profile_picture = models.ImageField(
        _("profile picture"), upload_to="picture/%y/%m/%d", default="default.png"
    )
